...
compare_zone <- function(
  ...
  descriptive <- varKrigDescr(data, variable, grid_dim = grid_dim)
  ...
  list(
    'differences' = comparisson_table$Diferencias,
...
validVarKrig <-
  function(
    ...
    descStatTable <- data.table::dcast(
      data = MedCV,
      n ~ Variable,
      value.var = "MedCV"
    )
    ...
    return(list(Diferencias = MeansComparissons, Descriptivo = descStatTable))
  }
...
#' @noRd
varKrigDescr <-
  function(datos_predsf, dataToCompare, grid_dim) {
    is_newData <- FALSE
...
makeMeanComparisson <-
  function(
    ...
    for (k in 1:nn) {
      ...
      sdtdif[k] <- descStat[
        which(descStat[["Variable"]] == VariableEstudiada),
        "MedianaK"
      ]
    ...
    MisLetras <-
      CalcLetras(myMeans = medias, pvalue = pvalue, alpha = alpha)
    if (retDMS) {
